...
void Table::_read_btv(int begin, int end) {
    for (int i = begin; i < end; i++) {      
        // if(config->btv_WAH_length_map[i] == 8)
        //     bitmaps[i] = new ibis::bitvector8();
        // else if(config->btv_WAH_length_map[i] == 16)
        //     bitmaps[i] = new ibis::bitvector16();
        // else if(config->btv_WAH_length_map[i] == 32)
        //     bitmaps[i] = new ibis::bitvector();
        MAKE_BITVECTOR(bitmaps[i], config->btv_WAH_length_map[i]);
                    
        if (config->INDEX_PATH != "")
            bitmaps[i]->read(getBtvName(i).c_str());
...
int Table::update(int tid, uint64_t rowid, int to_val) 
{
    ...
    if (config->on_disk) {
        ...
        bitvector1.read(getBtvName(from_val).c_str());
        bitvector2.read(getBtvName(to_val).c_str());
        // if (verbose)
        ...
        bitvector1.write(getBtvName(from_val).c_str());
        bitvector2.write(getBtvName(to_val).c_str());
        // if (verbose)
...
int Table::remove(int tid, uint64_t rowid) 
{
    ...
    if (config->on_disk) {
        ...
        bitvector.read(getBtvName(val).c_str());
        bitvector.setBit(rowid, 0, config);
        bitvector.write(getBtvName(val).c_str());
    }
    else {
        if (config->encoding == EE) {
...
int Table::evaluate(int tid, uint32_t val) 
{
    ...
    
    {
        ...
        if (config->on_disk) {
            string name = getBtvName(val);
            res->read(name.c_str());
      	} else {
        //     if (config->btv_WAH_length_map[val] == 8) {
        //         res = new ibis::bitvector8();
	    //     ibis::bitvector8* tmp = dynamic_cast<ibis::bitvector8*>(res);
		// tmp->copy(*(dynamic_cast<ibis::bitvector8*>(bitmaps[val])));
		// cnt = tmp->do_cnt();;
        //     }
        //     else if (config->btv_WAH_length_map[val] == 16) {
        //         res = new ibis::bitvector16();
	    //     ibis::bitvector16* tmp = dynamic_cast<ibis::bitvector16*>(res);
		// tmp->copy(*(dynamic_cast<ibis::bitvector16*>(bitmaps[val])));
		// cnt = tmp->do_cnt();
        //     }
	    // else if (config->btv_WAH_length_map[val] == 32) {
	    //     res = new ibis::bitvector();
	    //     ibis::bitvector* tmp = dynamic_cast<ibis::bitvector*>(res);
		// tmp->copy(*(dynamic_cast<ibis::bitvector*>(bitmaps[val])));
		// cnt = tmp->do_cnt();
        //     }
            MAKE_BITVECTOR(res, config->btv_WAH_length_map[val]);
