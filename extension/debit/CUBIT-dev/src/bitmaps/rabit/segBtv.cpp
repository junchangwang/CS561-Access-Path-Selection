    ...
    rows_per_seg(rhs.rows_per_seg), words_per_seg(rhs.words_per_seg), n_rows(rhs.n_rows)
{
    for (const auto & [id_t, seg_t] : rhs.seg_table) {
        btv_seg *seg = new btv_seg(*seg_t);
        seg_table[id_t] = seg;
    }
}

SegBtv::SegBtv(const SegBtv &rhs, int begin, int end) : encoded_word_len(rhs.encoded_word_len),
                                                        num_segs(end - begin),
                                                        rows_per_seg(rhs.rows_per_seg),
                                                        words_per_seg(rhs.words_per_seg),
                                                        n_rows(rhs.seg_table.at(end - 1)->end_row - rhs.seg_table.at(begin)->start_row)
{
    for (int idx = begin; idx < end; ++idx)
    {
        btv_seg *seg = new btv_seg(*rhs.seg_table.at(idx));
        seg_table[idx] = seg;
    }
}
...
int SegBtv::rangeDeepCopy(const SegBtv &rhs, int start, int end)
{
    for (uint32_t idx = start; idx < end; idx++)
    {
        seg_table[idx]->btv->copy(*rhs.seg_table.at(idx)->btv);
    }
    return 0;
}
...
btv_seg* SegBtv::getSeg(uint64_t row_id)
{
    ...
    seg = __atomic_load_n(&seg_table[idx], MM_ACQUIRE);
...
int SegBtv::adjustSize(uint64_t first, uint64_t second)
{
    ...
    if (n_rows_t + to_append <= rows_per_seg) {
        ...
        seg_table[idx] = new btv_seg(seg_new);
    }
...
void SegBtv::_and_parallel(SegBtv &rhs, Table_config *config)
{
    ...
    {
        threads[i] = thread(&SegBtv::_and_in_thread, this, &rhs, begin[i], begin[i + 1]);
    }
    for (int i = 0; i < n_threads; i++)
    {
        threads[i].join();
...
void SegBtv::_count_ones_in_thread(uint64_t *cnt, int begin, int end) {
    auto iter1 = seg_table.find(begin);
    auto iter2 = seg_table.find(end);
    for (; iter1 != iter2; iter1++)
        *cnt += iter1->second->btv->count_ones();
}

uint64_t SegBtv::do_cnt_parallel_withtimestamp(Table_config *config, const std::map<uint32_t, std::set<uint64_t>>& seg_rids, uint64_t l_timestamp) 
{
    uint64_t cnt = 0UL;
    ...
    for (int i = 0; i < n_threads; i++) {
        threads[i] = thread(&SegBtv::_cnt_in_thread_withtimestamp, this, &n_cnt[i], begin[i], begin[i + 1],\
                                l_timestamp, seg_rids, config);
    }
    for (int i = 0; i < n_threads; i++) {
        threads[i].join();
        cnt += n_cnt[i];
...
void SegBtv::_cnt_in_thread_withtimestamp(uint64_t *cnt, int begin, int end, uint64_t l_timestamp, const std::map<uint32_t, std::set<uint64_t>>& seg_rids, Table_config *config) 
{
    for(int i = begin; i < end; i++) {
        btv_seg *curr_seg = __atomic_load_n(&seg_table[i], MM_ACQUIRE);
        *cnt += curr_seg->btv->count_ones();

        // read rids
        std::set<uint64_t> rids{};
        curr_seg->buffer->get_rids_from_buffer(l_timestamp, rids);
        ...
        for (const auto &row_id_t : rids) {
            *cnt += curr_seg->btv->getBit(row_id_t - curr_seg->start_row, config) ? (-1) : (1);
        }
    }
}
                                         ...
                                         Table_config *config) 
{
    for(uint32_t i = begin; i < end; i++) {
        ...
        // b1 | (b2 XOR rids) => b1 XOR (b2 XOR rids) => (b1 XOR b2) XOR rids => !(b1 XOR b2)
        // given b1 = !b2  OR  b1 = b2 = 0
...
void SegBtv::Replacesegbtv(uint32_t seg_id, uint64_t l_timestamp, Table_config *config)
{
    btv_seg *seg_new = new btv_seg(*this->seg_table[seg_id]);
    seg_new->next = this->seg_table[seg_id];
    
    // Write buffer to rubs
    std::set<uint64_t> rids;
    this->seg_table[seg_id]->buffer->get_rids_from_buffer(l_timestamp, rids);

    // Write to new bitvector
    for(const auto &row_id_t : rids) {
        if(row_id_t <= seg_new->end_row && row_id_t >= seg_new->start_row)
            seg_new->setbit(row_id_t, 1, config);
    }
