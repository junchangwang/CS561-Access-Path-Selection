...
uint64_t Table::range(int tid, uint32_t start, uint32_t range) {
    ibis::bitvector res;
    ...
    if(config->encoding == EE) {
        for(uint32_t i = start; i < start + range; i++) {
            lk_bitmaps[i]->lock_shared(); // To synchronize with updates and deletes.
        }
        for(uint32_t i = start; i < start + range; i++) {
            ibis::bitvector * c_res = (*bitmaps[i] ^ *update_bitmaps[i]);
            res |= *c_res;
            res.decompress();
            delete c_res;
        }
        ...
        if(start == 0) {
            lk_bitmaps[range - 1]->lock_shared();
            res.copy(*bitmaps[range - 1]);
            res ^= *update_bitmaps[range - 1];
            lk_bitmaps[range - 1]->unlock_shared();
        }
        else {
            uint32_t l = start - 1;
            uint32_t r = start + range - 1;
            for(int idx = l; idx <= r; idx++) {
                lk_bitmaps[idx]->lock_shared();
            }
            res ^= *bitmaps[l];
            res.decompress();
            res ^= *update_bitmaps[l];
            res.decompress();
            res ^= *bitmaps[r];
            res.decompress();
            res ^= *update_bitmaps[r];
            res.decompress();
            for(int idx = l; idx <= r; idx++) {
                lk_bitmaps[idx]->unlock_shared();
            }
        }
    }
    else {
